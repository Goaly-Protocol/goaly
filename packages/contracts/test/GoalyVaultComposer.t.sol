...
contract GoalyVaultComposerTest is Test {
    ...
    function setUp() public {
        ...
        composer = new GoalyVaultComposer(
            IERC20(address(usdt0)), IGoalyVaultDeposit(address(vault)), endpoint, oft
        );
    }

    function _composeMessage(uint256 amount, address recipient)
        internal
        pure
    ...
    function test_RejectsUnexpectedOft() public {
        ...
        composer.lzCompose(
            address(0xBAD), keccak256("g"), _composeMessage(100 * UNIT, user), address(0), ""
        );
    }
}
